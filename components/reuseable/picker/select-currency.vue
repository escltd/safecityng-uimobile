<template>
  <div class="flex flex-col mt-4">
    ...
    <div class="flex flex-row gap-3 rounded-lg text-gray items-center relative">
      <input v-model="searchParams.name" @click="clickSearch" @keyup="pressEnter" type="text" required class="caret-black p-2 bg-#0001 text-[0.9rem] <smw-full border-width-0 w-100% text-black"/>
    ...
    <div class="cursor-pointer caret-transparent relative w-100% z-99">
      ...
      :class="searchResults.length>0?'min-h-[25px]':''">
        ...
        <div v-for="(result, index) in searchResults" :key="index" class="p-y-1 w-100% flex hover:text-white hover:bg-dark cursor-pointer rounded-b" @click="setValue(result.name)">
          <span class="p-x-2">{{result.name}}</span>
        </div>
      </div>
    </div>
  </div>
...
const initialstate = reactive({ workflow:'enabled', name: props.modelValue, page:0, limit: 50 })
...
const getSearchResults = async () => {
  ...
  searchResults.value =[]
  const response = await HTTP().post('/api/currencys/search', searchParams).then(
  (response) => {
    return response
  }).catch((error) => {
