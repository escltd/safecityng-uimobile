...
import { showAlert } from '~/plugins/sweetalert';
  ...
  const getSearchResults = async () => {
    ...
    searchResults.value =[]
    const response = await HTTP().post('/api/bankaccounts/search', searchParams).then(
    (response) => {
      return response
    }).catch((error) => {
    ...
    if (response.data.length === 0) {
      showAlert({
        titleText:'Create Bank Account',
        text:"You have not created "+props.currency+" Bank account.",
        icon:'info',
      })
    }
  }
