...
import { showAlert } from '~/plugins/sweetalert';
  ...
  const getSearchResults = async () => {
    ...
    searchResults.value =[]
    const response = await HTTP().post('/api/bankaccounts/allocate/search', searchParams).then(
    (response) => {
      return response
    }).catch((error) => {
    ...
    if (response.data.length === 0) {
      showAlert({
        titleText:'No System Account Available',
        text:"No "+props.currency+" System Bank account.",
        icon:'info',
      })
    }
  }
