  ...
  <div>
    <Modal class="bg-zinc-100">
      <div class="flex flex-row items-center w-100% justify-between">
        <div class="font-extrabold text-1rem pb-1">Update Bank Account</div>
...
let props = defineProps(["bankaccount"]);
...
const initialState = reactive({
  ...
  accountname: props?.bankaccount?.accountname,
  ...
  bankaddress: props?.bankaccount?.bankaddress,
});
...
const resetForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    let response = await HTTP().put('/api/bankaccount/'+props?.bankaccount?.id, formData).then(
      (response) => {
        return response;
      }).catch((error) => {
