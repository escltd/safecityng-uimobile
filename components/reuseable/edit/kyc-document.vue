  ...
  <div>
    <Modal class="bg-zinc-100">
      <div class="flex flex-row items-center w-100% justify-between">
        <div class="font-extrabold text-1rem pb-1">Update KYC Document</div>
      ...
      <div class="flex flex-row justify-center items-center p-t-5">
        <div class="p-1 rounded bg-zinc-100 text-dark cursor-pointer" @click="fileUploader.click()">
            <input class="hidden" accept="image/png, image/jpeg, image/jpg, image/gif, .csv, application/pdf, application/msword"
...
let props = defineProps(["kycDocument"]);
...
const initialState = reactive({
  code: props?.kycDocument?.code,
  doctype: props?.kycDocument?.doctype,
  filename: props?.kycDocument?.filename,
  filepath: props?.kycDocument?.filepath,
  filesize: props?.kycDocument?.filesize,
  filetype: props?.kycDocument?.filetype,
  issuedby: props?.kycDocument?.issuedby,
  validuntil: props?.kycDocument?.validuntil,
  workflow: "pending",
});
...
const resetForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    let response = await HTTP().put('/api/document/'+props?.kycDocument?.id, formData).then(
      (response) => {
        return response;
      }).catch((error) => {
